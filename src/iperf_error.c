...
iperf_strerror(int int_errno)
{
	...
	case IENOSCTP:
        ...
        case IEMESSAGE:
            ...
            break;
        case IESENDMESSAGE:
            snprintf(errstr, len, "unable to send control message - port may not be available, the other side may stopped running, etc.");
            perr = 1;
            break;
        case IERECVMESSAGE:
            snprintf(errstr, len, "unable to receive control message - port may not be available, the other side may stopped running, etc.");
            perr = 1;
            break;
        case IESENDPARAMS:
