...
int iperf_accept(struct iperf_test *test) {
  ...
  if (test->ctrl_sck == -1) {
    ...
    // Set Control Connection TCP Keepalive (especially useful for long UDP test
    // sessions)
...
int iperf_handle_message_server(struct iperf_test *test) {
  ...
  if (test->debug_level >= DEBUG_LEVEL_INFO) {
    iperf_printf(test,
                 "Reading new State from the Client - current state is %d-%s\n",
  ...
  if (test->debug_level >= DEBUG_LEVEL_INFO) {
    iperf_printf(test,
                 "State change: server received and changed State to %d-%s\n",
                 test->state, state_to_text(test->state));
  }
        ...
        if (test->state == IPERF_START) {
          if (test->settings->idle_timeout > 0 &&
            ...
            if (iperf_get_test_one_off(test)) {
              if (test->debug)
                printf("No connection request was received for %d sec in "
                       "one-off mode; exiting.\n",
                       test->settings->idle_timeout);
              exit(0);
            }
