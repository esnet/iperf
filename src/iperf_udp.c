...
iperf_udp_recv(struct iperf_stream *sp)
{
    ...
    r = Nread(sp->socket, sp->buffer, size, Pudp, sp->test);
	...
	if (sp->test->debug > 1)
	    iperf_err(sp->test, "udp pcount %" PRIu64 " packet_count %d\n", pcount, sp->packet_count);
...
iperf_udp_listen(struct iperf_test *test)
{
    ...
    if ((s = netannounce(test->settings->domain, Pudp, test->bind_address, test->bind_dev,
...
iperf_udp_connect(struct iperf_test *test)
{
    ...
    for (i = 0; i<30; i++) {
        fd_set read_fds;
        FD_ZERO(&read_fds);            //Zero out the file descriptor set
        FD_SET(s, &read_fds);     //Set the current socket file descriptor into the set
